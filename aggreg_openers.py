...
output_file = f"{data_dir}/team_home_field_by_year/first_inning_nb_results_2018_2024.csv"
...
# --- Aggregate all series opener games ---
all_games = []
...
for year in years:
    ...
    games["home_travel"] = pd.to_numeric(home_travel, errors="coerce")
    games["vis_travel"] = pd.to_numeric(vis_travel, errors="coerce")

    # --- Only keep series openers (both travels > 0) ---
    games = games[(games["home_travel"] > 0) & (games["vis_travel"] > 0)].copy()
    all_games.append(games)

# --- Concatenate all years ---
all_games_df = pd.concat(all_games, ignore_index=True)
print(f"\nTotal series openers across 2018-2024: {len(all_games_df)} games")

# --- Loop over each team (ignore TBA) ---
all_teams = sorted(set(all_games_df["hometeam"]).union(set(all_games_df["visteam"])))
teams = [t for t in all_teams if t != "TBA"]

results = []

for team in teams:
    team_games = all_games_df[
        (all_games_df["visteam"] == team) & 
        (all_games_df["hometeam"] != "TBA")
    ].copy()
    if team_games.empty:
        continue
    ...
    home_df = home_df[home_df['team_id'] != "TBA"]
    home_df['is_home'] = 1
    ...
    away_df = away_df[away_df['team_id'] != "TBA"]
    away_df['is_home'] = 0
