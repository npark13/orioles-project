...
output_file = f"{data_dir}/team_home_field_by_year/first_inning_nb_results_home_2018_2024.csv"
...
# --- Loop over each home team (ignore TBA) ---
all_teams = sorted(set(all_games_df["hometeam"]))
teams = [t for t in all_teams if t != "TBA"]

results = []

for team in teams:
    team_games = all_games_df[all_games_df["hometeam"] == team].copy()
    if team_games.empty:
        continue
