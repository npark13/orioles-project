...
output_file = "/Users/kevinhe/orioles-project/data/out/first_inning_nb_results_with_travel_angels_combined.csv"
...
for year in years:
    ...
    # --- Filter for Anaheim Angels games ---
    angels_games = games[(games['hometeam']=='ANA') | (games['visteam']=='ANA')].copy()
    if angels_games.empty:
        print(f"No Anaheim Angels games for {year}, skipping")
        continue

    # --- Extract game dates ---
    angels_games["date"] = angels_games["game_id"].apply(lambda x: pd.to_datetime(x[3:11], format="%Y%m%d"))
    angels_games = angels_games.sort_values("date").reset_index(drop=True)

    # --- Track last locations ---
    angels_games["home_last_location"] = "N/A"
    angels_games["visiting_last_location"] = "N/A"
    last_location = {}

    for idx, row in angels_games.iterrows():
        home = row["hometeam"]
        away = row["visteam"]

        if home in last_location:
            angels_games.at[idx, "home_last_location"] = last_location[home]
        last_location[home] = home

        if away in last_location:
            angels_games.at[idx, "visiting_last_location"] = last_location[away]
        last_location[away] = home

    # --- Compute travel distances ---
    home_travel, vis_travel = [], []

    for _, row in angels_games.iterrows():
    ...
    angels_games["home_travel"] = pd.to_numeric(home_travel, errors="coerce") + 1e-6
    angels_games["vis_travel"] = pd.to_numeric(vis_travel, errors="coerce") + 1e-6

    # --- Filter only valid travel rows ---
    angels_games = angels_games[
        ~((angels_games['home_last_location']=="N/A") | (angels_games['visiting_last_location']=="N/A"))
    ].copy()

    # --- Subsets ---
    subsets = {
        "all": angels_games,
        "home": angels_games[angels_games['hometeam']=="ANA"],
        "visiting": angels_games[angels_games['visteam']=="ANA"]
    }
    ...
    for subset_name, subset_df in subsets.items():
        ...
        home_df = subset_df[['game_id','hometeam','home_first_inning_runs','vis_ERA','home_travel']].rename(
            columns={'hometeam':'team_id','home_first_inning_runs':'first_inning_runs','vis_ERA':'opp_ERA','home_travel':'travel'}
        )
        home_df['is_home'] = 1

        away_df = subset_df[['game_id','visteam','visiting_first_inning_runs','home_ERA','vis_travel']].rename(
